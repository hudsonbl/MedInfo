...
router.get('/:id', requireAuthentication, async(req, res) => {
    ...
    if(req.authenticated) { //TODO: this will actually check for authentic user in future
        ...
        debug("-- Getting lab reports failed unauthenticated user");
        res.status(400).send({
...
router.post('/:id', requireAuthentication, async(req, res) => {
    ...
    if(validateAgainstSchema(body, LabReportSchema), req.authenticated){
        try{
...
router.delete('/:userId/:reportId', requireAuthentication, async (req, res) => {
